...
<a href="{{ route('admin.sekolah.detail-data-sarpras', $sekolah) }}"
    class="{{ Request::routeIs('admin.sekolah.detail-data-sarpras') ? 'border-b-2 border-yellow-400 bg-[#FFB930] text-black font-medium' : 'text-gray-600 hover:bg-yellow-50/50 hover:text-black' }} px-6 py-3 transition-all duration-200">
    Detail Data Sarpras
</a>
                    </div>
...
<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
    ...
    @foreach($groupedSarpras as $jenisRuangKey => $details)
        ...
        <div class="room-button block text-center p-4 rounded-lg bg-white border border-yellow-200 hover:bg-yellow-50 transition-all duration-200 shadow-sm">
            ...
            <a href="{{ route('admin.sekolah.show.detail-sarpras', ['sekolah' => $sekolah->id, 'detailSarpras' => $firstDetail->id]) }}"
                        ...
                        @if(isset($selectedDetail))
                            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
                                    ...
                                    <!-- Form Update Data -->
                                    <div class="space-y-4">
                                        <form action="{{ route('admin.sekolah.update-detail-sarpras', ['sekolah' => $sekolah->id, 'detailSarpras' => $selectedDetail->id]) }}" 
                                              method="POST" 
                                              class="space-y-4">
                                            @csrf
                                            @method('PUT')
...
<div id="addRoomModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form action="{{ route('admin.sekolah.store-detail-sarpras', $sekolah->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
